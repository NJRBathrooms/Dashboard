// ============================================================
// NJR Bathrooms — verificação diária Insurance & W9 (subcontratados)
// Chamada de dentro do rentals-cron (1 função serverless a menos —
// limite de 12 no plano Hobby). Olha o cadastro mais recente de cada
// empresa e avisa o Nilmar quando o seguro (Workers' Comp) vence em
// <=30 dias ou já venceu. Dedupe via Alerted30/AlertedExpired,
...
async function runInsuranceCheck() {
  ...
  if (!expired.length && !expiring.length) return { expired: 0, expiring: 0 };
...
<div style="font-family:Arial,sans-serif;max-width:720px;color:#1a2b3c">
  ...
  const sh = index.find(s => s.title === SUBPROF_SHEET);
  if (sh) {
...
module.exports = { runInsuranceCheck };
